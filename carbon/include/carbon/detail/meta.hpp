...
namespace carbon { namespace detail {
    ...
    template<typename T>
    struct is_serializer_specialized<T, std::void_t<typename T::serializer_type>> : std::true_type {
    };
    ...
    template<class P>
    struct is_input_proxy : public std::is_same<typename P::proxy_tag, input_proxy_tag> {};

    template<class P>
    struct is_output_proxy : public std::is_same<typename P::proxy_tag, output_proxy_tag> {};
