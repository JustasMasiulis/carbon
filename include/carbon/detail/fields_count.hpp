...
namespace carbon::detail::pfr {
    ...
    struct ubiq_constructor {
        std::size_t ignore;
        template<class T>
        constexpr operator T&() const noexcept; // Undefined, allows initialization of
                                                // reference fields (T& and const T&)

        // template <class Type> constexpr operator Type&&() const noexcept;
        // // Undefined, allows initialization of rvalue reference fields
        // and move-only types
    };

    ///////////////////// Structure that can be converted to reference to anything except
    /// reference to T
    template<class T>
    struct ubiq_constructor_except {
        template<class T>
        constexpr operator std::enable_if_t<!std::is_same<T, T>::value, T&>() const
            noexcept; // Undefined
    };
    ...
    // that there's no constructor from `decltype(ubiq_constructor{I})...` Special case
    // for N == 1: `std::is_constructible<T, ubiq_constructor>` returns true if N == 1 and
    // T is copy/move constructible.
    template<class T, std::size_t N>
    struct is_aggregate_initializable_n {
        template<std::size_t... Is>
        static constexpr bool is_not_constructible_n(std::index_sequence<Is...>) noexcept
        {
            return !std::is_constructible_v<T, decltype(ubiq_constructor{ Is })...> ||
                   is_single_field_and_aggregate_initializable<N, T>::value;
        }

        static constexpr bool value =
            std::is_empty_v<T> || std::is_fundamental_v<T> ||
    ...
    constexpr auto enable_if_constructible_helper(std::index_sequence<I...>) noexcept
        -> std::add_pointer_t<decltype(T{ ubiq_constructor{ I }... })>;
    ...
    ///////////////////// Non greedy fields count search. Templates instantiation depth is
    /// log(sizeof(T)), templates instantiation count is log(sizeof(T)).
    template<class T, std::size_t N>
    ...
    detect_fields_count(size_t_<Begin>, size_t_<Middle>, int) noexcept;

    template<class T, std::size_t Begin, std::size_t Middle>
    constexpr enable_if_constructible_helper_t<T, Middle>
    detect_fields_count(size_t_<Begin>, size_t_<Middle>, long) noexcept
    {
    ...
    ///////////////////// Greedy search. Templates instantiation depth is log(sizeof(T)),
    /// templates instantiation count is log(sizeof(T))*T in worst case.
    template<class T, std::size_t N>
    constexpr enable_if_constructible_helper_t<T, N>
    detect_fields_count_greedy_remember(size_t_<N>, long) noexcept
    {
        return N;
    }
    ...
    {
        constexpr std::size_t middle = Begin + (Last - Begin) / 2u;
    ...
    template<class T, std::size_t N>
    constexpr decltype(sizeof(T{})) detect_fields_count_dispatch(size_t_<N>,
                                                                 long) noexcept
    {
    ...
    constexpr std::size_t detect_fields_count_dispatch(size_t_<N>, int) noexcept
    {
        ...
        // initializations for T up to N parameters and return the bigest succeeded (we
        // can not use binary search for detecting fields count).
    ...
    constexpr std::size_t fields_count() noexcept
    {
        ...
        static_assert(std::is_copy_constructible_v<std::remove_all_extents_t<type>>,
        ...
        static_assert(
            std::is_aggregate<type>::value // Does not return `true` for build in types.
                || std::is_standard_layout<type>::value, // Does not return `true` for
                                                         // structs that have non standard
                                                         // layout members.
        ...
        constexpr std::size_t result =
            detect_fields_count_dispatch<type>(size_t_<max_fields_count>{}, 1L);
        ...
        static_assert(
            result != 0 || std::is_empty_v<type> || std::is_fundamental_v<type> ||
                std::is_reference_v<type>,
